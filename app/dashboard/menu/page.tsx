...
export default function MenuManagerPage() {
  ...
  return (
    <div>
      <TopHeader 
        title="Café Menu & Allergens Manager" 
        subtitle="Manage Food/Beverage Items, Vegetarian & Vegan Flags, and Approved Allergens" 
      />
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.35rem' }}>
                  ...
                  <span style={{ fontSize: '1.15rem', fontWeight: 800, color: 'var(--accent-amber)' }}>€{item.price.toFixed(2)}</span>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
                  {item.is_vegetarian && <span style={{ fontSize: '0.72rem', background: 'rgba(16, 185, 129, 0.2)', color: 'var(--accent-emerald)', padding: '0.15rem 0.5rem', borderRadius: '4px', fontWeight: 600 }}>🌱 Vegetarian</span>}
                  {item.is_vegan && <span style={{ fontSize: '0.72rem', background: 'rgba(6, 182, 212, 0.2)', color: 'var(--accent-cyan)', padding: '0.15rem 0.5rem', borderRadius: '4px', fontWeight: 600 }}>🌿 Vegan</span>}
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Approved Allergens: <strong>{item.approved_allergens.join(', ') || 'None'}</strong></span>
                </div>
              </div>
      ...
      {showAddModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div className="glass-card" style={{ width: '480px', padding: '1.75rem' }}>
            <h3 style={{ fontSize: '1.25rem', marginBottom: '1rem' }}>Add New Café Item</h3>
            ...
            <div className="form-group">
              ...
              <input type="number" step="0.10" className="form-input" value={newItem.price} onChange={(e) => setNewItem({ ...newItem, price: Number(e.target.value) })} />
            </div>
            ...
            <div className="form-group">
              <label className="form-label">Approved Allergens (Comma separated)</label>
              <input type="text" className="form-input" placeholder="e.g. Gluten, Melk, Noten" value={newItem.approved_allergens} onChange={(e) => setNewItem({ ...newItem, approved_allergens: e.target.value })} />
            </div>

            <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '1.25rem' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.85rem' }}>
                <input type="checkbox" checked={newItem.is_vegetarian} onChange={(e) => setNewItem({ ...newItem, is_vegetarian: e.target.checked })} /> Vegetarian
              </label>
              <label style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.85rem' }}>
                <input type="checkbox" checked={newItem.is_vegan} onChange={(e) => setNewItem({ ...newItem, is_vegan: e.target.checked })} /> Vegan
              </label>
            </div>
