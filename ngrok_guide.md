# Optional ngrok Local Testing Guide for Meta Webhooks

This guide explains how to optionally expose your local Next.js server (`http://localhost:3000`) to the public internet using `ngrok` for testing Meta Instagram webhooks and OAuth callbacks.
...
META_OAUTH_REDIRECT_URI=https://a1b2-34-56-78-90.ngrok-free.app/api/auth/instagram/callback
```
...
2. In **Facebook Login for Business** -> **Settings**, set **Valid OAuth Redirect URIs** to:
...
4. Set **Verify Token** to the value of `META_WEBHOOK_VERIFY_TOKEN` (e.g., `ghent_cafe_secure_webhook_verify_token_2026`).
